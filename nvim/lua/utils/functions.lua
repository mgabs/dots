...
---@return table
M.load_user_config = function(config_path)
    local home = os.getenv("XDG_CONFIG_HOME") or os.getenv("HOME") or os.getenv("USERPROFILE") or
                     (os.getenv("HOMEDRIVE") .. os.getenv("HOMEPATH"))
    local config_file = home .. M.path_separator() .. config_path
...
function M.scratchRedir()
    vim.ui.input({
        ...
        local cmd = vim.api.nvim_exec(input, {
            output = true
        })
        local output = {}
